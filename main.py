...
class PatchRaptorClient(discord.Client):
    ...
    async def on_ready(self):
        ...
        try:
            ...
            logger.info_system("RaptorChat will start automatically after a short delay...")
    ...
    async def _periodic_player_update(self):
        ...
        # Track last log scan time
        last_log_scan = time.time()
        log_scan_interval = 1800  # 30 minutes
        ...
        while True:
            try:
                ...
                # Checks if server rotated to a new log file
                current_time = time.time()
                if current_time - last_log_scan > log_scan_interval:
                    logger.debug_system("Running periodic log file detection (30m interval)")
                    await self.command_handler.player_manager.update_server_log_files()
                    last_log_scan = current_time
...
def build_app():
    ...
    backup_manager = BackupManager(backup_path, backup_retention)
    ...
    default_raptorchat_path = os.path.join(default_raptorchat_dir, "RaptorChat.py")  # or "RaptorChat.exe" for compiled
