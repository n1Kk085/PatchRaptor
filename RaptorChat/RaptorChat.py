...
log_threads = []
DYNAMIC_CHECK_INTERVAL = 30 * 60  # 30 minutes in seconds
dynamic_check_lock = threading.Lock()  # Lock to prevent race condition between threads
last_global_dynamic_check = 0  # Timestamp of last dynamic check
...
def follow_log(server):
    ...
    global last_global_dynamic_check, DYNAMIC_CHECK_INTERVAL, dynamic_check_lock
    ...
    while not shutdown_event.is_set():
        try:
            # Get current log file from config
            ...
            # If log file path changed
            config_log_file = current_config.get("server_log_path")
            if current_log_file != config_log_file:
                if file_handle:
                    file_handle.close()
                    file_handle = None
                current_log_file = config_log_file
                logger.log(f"Now reading from log file: {current_log_file} for {server_name}", "DEBUG")
            
            # Open file if needed
            if file_handle is None and current_log_file and os.path.exists(current_log_file):
                try:
            ...
            # Read from file if we have a handle
            if file_handle:
                try:
                    ...
                    logger.log(f"Error reading from log file {current_log_file}: {e}", "ERROR")
                    file_handle = None
                    time.sleep(5)
            else:
                time.sleep(5)  # Wait before retrying if we couldn't open the file
            
            # Check for dynamic log file updates (every 30 minutes)
            current_time = time.time()
            if current_time - last_global_dynamic_check >= DYNAMIC_CHECK_INTERVAL:
                with dynamic_check_lock:
                    if current_time - last_global_dynamic_check >= DYNAMIC_CHECK_INTERVAL:
                        update_server_log_files()
                        last_global_dynamic_check = current_time
...
def handle_line(server, line):
    ...
    logger.log(f"CHAT: {relay_message}", "INFO")
...
def get_logs_directory():
    ...
    # Try finding an explicit log_dir in config first
    for s in SERVERS:
        if s.get("log_dir") and os.path.exists(s["log_dir"]):
            return s["log_dir"]

    # Try finding a server with a log_file
    for s in SERVERS:
        if s.get("server_log_path") and os.path.exists(os.path.dirname(s["server_log_path"])):
             return os.path.dirname(s["server_log_path"])
             
    # Try deriving from server_save_path
    for s in SERVERS:
        save_path = s.get("server_save_path")
        if save_path:
             ...
             logger.log(f"Checking derived logs dir: {logs_dir} (from {save_path})", "DEBUG")
             
             if os.path.isdir(logs_dir):
                 logger.log(f"Found logs directory: {logs_dir}", "INFO")
                 return logs_dir
                 
    return None

# Raw string patterns for server identification
MAP_PATTERNS = [
    r"ScorchedEarth",
    r"Ragnarok",
    r"TheIsland",
...
SERVER_NAME_PATTERNS = [
    r"Server:\s*\"([^\"]+)\".*has successfully started",  # Server startup message
    r"SessionName=([^\?&]+)",  # Session name parameter
    r"-SessionName\s+([^\s]+)"  # Session name flag
]

RCON_PATTERNS = [
    r"RCONPort=(\d+)",  # Command line parameter
    r"RCONPort\s+(\d+)",  # Alternative format
    r"-RCONPort\s+(\d+)",  # Command line flag
...
def extract_server_info_from_log(log_file_path):
    ...
    try:
        ...
        rcon_patterns = [
            r"RCONPort=(\d{4,5})\b",  # RCONPort=27103
            r"-RCONPort[= ](\d{4,5})\b",  # -RCONPort 27103 or -RCONPort=27103
            ...
            if not server_info["rcon_port"]:
                for pattern in rcon_patterns:
                    ...
                    if match:
                        ...
                        # Only accept ports in a reasonable range
                        if 1000 <= port <= 65535:
        ...
        logger.log(f"Error reading log file {log_file_path}: {e}", "ERROR")
        return None
...
def find_matching_log_file(server_config, exclude_files=None):
    ...
    if server_config.get("rcon_port"):
        expected_rcon_port = int(server_config["rcon_port"])
    else:
        expected_rcon_port = None
    ...
    if not log_files:
        if exclude_files:
            logger.log(f"No available ShooterGame_*.log files found for {server_name} (some may be assigned to other servers)", "WARNING")
        else:
    ...
    for log_file in log_files:
        server_info = extract_server_info_from_log(log_file)
        if server_info and server_info.get("rcon_port") == expected_rcon_port:
            logger.log(f"Found matching log file for {server_name}: {os.path.basename(log_file)}", "INFO")
            return log_file
    
    logger.log(f"No log file with matching RCON port found for {server_name}", "WARNING")
    return None
...
def update_server_log_files():
    ...
    try:
        ...
        for i, server in enumerate(SERVERS):
            ...
            logger.log(f"Searching for correct log file for server: {server_name}", "INFO")
...
if __name__ == "__main__":
    try:
        ...
        logger.log("Detecting log files...", "INFO")
        update_server_log_files()
        last_global_dynamic_check = time.time()
