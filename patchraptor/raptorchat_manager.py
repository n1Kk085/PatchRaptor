...
class RaptorChatManager:
    ...
    def start(self, is_delayed_start: bool = False) -> bool:
        ...
        try:
            ...
            if getattr(sys, 'frozen', False):
                ...
                self.process = subprocess.Popen([exe_path], cwd=self.raptorchat_dir or None)
    ...
    def stop(self) -> bool:
        ...
        if not self.is_running():
            ...
            return False
        try:
            # INSTANT KILL REQUEST: Immediately force kill by name on Windows
            if sys.platform == "win32":
                 logger.debug_system("Executing instant taskkill for RaptorChat.exe")
                 try:
                    subprocess.run(
                        ["taskkill", "/F", "/IM", "RaptorChat.exe"], 
                        stdout=subprocess.PIPE, stderr=subprocess.PIPE,
                        creationflags=subprocess.CREATE_NO_WINDOW
                    )
                 except Exception as e:
                     logger.debug_system(f"Instant taskkill failed: {e}")
            
            # Continue with process object cleanup just in case
            if self.process:
                self.process.kill()
            
            # Skip the graceful terminate/wait logic entirely since we nuked it
