...
class UpdateManagementHandler:
    ...
    async def _internal_shutdown_and_update_servers(self, channel, skip_countdown=False):
        ...
        await self.discord_manager.send_temp_message(channel, "🛑 Starting server shutdown sequence...")
        ...
        # Build and execute SteamCMD command directly mirroring the manual success path
        # Note: We omit +force_install_dir to prevent path-conflict 0x6 errors on Windows 11.
        cmd_args = [
        ...
        if servers_online:
            ...
            # Use the 120s recovery delay from .forceupdate for consistency
            last_online_time = asyncio.get_event_loop().time()
            remaining_delay = 120
            
            logger.debug_update(f"Entering 120s recovery wait for .update")
            await asyncio.sleep(remaining_delay)
            
            # Resume PlayerManager
            if hasattr(self, 'player_manager') and self.player_manager:
            ...
            # Webhook notification for completion (Only for standard updates, not force updates)
            if not skip_countdown:
                ...
                logger.debug_update("Skipping webhook notification for force update")
        else:
    ...
    async def _internal_cmd_forceupdate(self, message, content: str, content_lower: str):
        ...
        # Case 1: Global Force Update (.forceupdate)
        if len(parts) == 1:
            # Check if update is already in progress
            if self.update_lock.locked():
                await self.discord_manager.send_temp_message(message.channel, "⚠️ Update already in progress.")
                return
            ...
            async with self.update_lock:
                ...
                try:
                    ...
                    logger.info_system("Starting unified global force update process...")
                    await self.discord_manager.send_temp_message(message.channel, "🦖 Force update started. Bypassing countdown and shutting down servers...")
                    
                    # Execute the unified update logic immediately (skipping the 15m countdown)
                    await self._internal_shutdown_and_update_servers(message.channel, skip_countdown=True)
                    return # Exit once global update is complete
                    
                except asyncio.CancelledError:
                    logger.info_system("Force update process cancelled")
                except Exception as e:
                    logger.error_system(f"Critical error during force update: {e}")
                    await self.discord_manager.send_temp_message(message.channel, f"❌ **Force Update Failed:** {str(e)}")
                finally:
                    if os.path.exists("update_in_progress.json"):
                        os.remove("update_in_progress.json")
            return
        
        # Case 2: Map-Specific Force Update (.forceupdate <map>)
        map_name = parts[1]
        # Rest of map-specific legacy logic follows...
        server_dir = self.config_manager.get("server_dir")
        steamcmd_path = self.config_manager.get("steamcmd_path")
        app_id = self.config_manager.get("app_id")
        
        # Find the server object
        server = next((s for s in self.server_manager.servers if s.name == map_name), None)
        if not server:
            await self.discord_manager.send_temp_message(message.channel, f"⚠️ Server {map_name} not found.")
            return
        ...
        if self.server_manager.is_specific_server_running(server):
            logger.debug_update(f"Server {map_name} is running, shutting down for update")
            await self.discord_manager.send_temp_message(message.channel, f"🛑 Shutting down {display_name} for update...")
            self.server_manager.stop_server(server)
            await asyncio.sleep(5)
        else:
            logger.debug_update(f"Server {map_name} not running, proceeding with update")
    ...
    async def _shutdown_all_servers(self, channel=None):
        ...
        if channel:
            await self.discord_manager.send_temp_message(channel, "📡 Sending RCON shutdown commands...")
        ...
        for server in self.server_manager.servers:
            ...
            if self.server_manager.is_specific_server_running(server):
                ...
                logger.debug_update(f"Server {server.name} is not running, skipping DoExit")
        ...
        for server, task in tasks:
            ...
            try:
                ...
                if channel:
                    await self.discord_manager.send_temp_message(channel, f"✅ Successfully sent shutdown to {display_name}")
                ...
                if channel:
                    await self.discord_manager.send_temp_message(channel, f"❌ Failed to send shutdown to {display_name}: {e.reason}")
        
        # Wait for all servers to shut down
        logger.debug_update("Waiting for all servers to shut down")
        if channel:
            await self.discord_manager.send_temp_message(channel, "⏳ Waiting for all servers to shut down completely...")
