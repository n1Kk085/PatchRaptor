...
class UpdateManagementHandler:
    ...
    async def _shutdown_and_update_servers(self, channel):
        ...
        if hasattr(self, 'raptorchat_manager') and self.raptorchat_manager:
            ...
            # Use delayed start to let servers initialize
    ...
    async def cmd_update(self, message, content: str, content_lower: str):
        ...
        if os.path.exists("update_in_progress.json"):
            ...
            try:
                ...
                if (asyncio.get_event_loop().time() - creation_time) > 7200: # 2 hours
    ...
    async def cmd_forceupdate(self, message, content: str, content_lower: str):
        ...
        if len(parts) == 1:
            ...
            if running_servers:
                ...
                if servers_online:
                    ...
                    # Calculate remaining delay (5 minutes from now)
                    remaining_delay = max(0, 300 - (asyncio.get_event_loop().time() - last_online_time))
        ...
        try:
            ...
            if hasattr(self, 'player_manager'):
                ...
                self.player_manager.clear_server_players(server.name)
                self.player_manager.reset_file_positions(server.name)
    ...
    async def start_autoupdate_checker(self, discord_channel=None):
        ...
        logger.debug_update("Creating autoupdate checker task")
        logger.info_system("Starting autoupdate checker...")
        self.autoupdate_task = asyncio.create_task(self._autoupdate_checker_loop(discord_channel))
    ...
    async def _autoupdate_checker_loop(self, discord_channel=None):
        ...
        logger.debug_update("Waiting 10 minutes before first autoupdate check")
        logger.info_system("Waiting 10 minutes before first autoupdate check...")
        ...
        while True:
            try:
                ...
                logger.debug_update("Waiting 10 minutes before next check")
                await asyncio.sleep(600)
                logger.debug_update("10-minute wait completed, continuing loop")
