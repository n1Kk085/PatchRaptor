"""
Test suite for Update Management Handler - validates update, forceupdate, and autoupdate commands.
"""
import pytest
import asyncio
import os
...
def handler_mocks(mock_server):
    ...
    version_manager.get_latest_build_id = AsyncMock()
    ...
    raptorchat_manager.start = AsyncMock()
...
async def update_handler(handler_mocks):
    ...
    handler = UpdateManagementHandler(
        server_manager=handler_mocks['server_manager'],
        rcon_manager=handler_mocks['rcon_manager'],
        version_manager=handler_mocks['version_manager'],
        discord_manager=handler_mocks['discord_manager'],
        config_manager=handler_mocks['config_manager'],
        player_manager=handler_mocks['player_manager'],
        raptorchat_manager=handler_mocks['raptorchat_manager']
    )
    yield handler
...
class TestRestartMethods:
    ...
    async def test_restart_all_servers(self, update_handler, handler_mocks):
        ...
        with patch('asyncio.sleep', new_callable=AsyncMock):
            with patch('patchraptor.update_management_handler.RaptorChatUtils.delayed_raptorchat_restart', new_callable=AsyncMock):
    ...
    @pytest.mark.asyncio
    async def test_wait_for_servers_online(self, update_handler, handler_mocks, mock_server):
        """Test waiting for servers to come online."""
        handler_mocks['server_manager'].is_specific_server_running = Mock(return_value=True)
        
        with patch('asyncio.sleep', new_callable=AsyncMock):
            result = await update_handler._wait_for_servers_online([mock_server], timeout=10, check_interval=1)
        
        assert result is True
    
    @pytest.mark.asyncio
    async def test_wait_for_servers_online_timeout(self, update_handler, handler_mocks, mock_server):
        """Test timeout waiting for servers."""
        handler_mocks['server_manager'].is_specific_server_running = Mock(return_value=False)
        
        with patch('asyncio.sleep', new_callable=AsyncMock):
            result = await update_handler._wait_for_servers_online([mock_server], timeout=2, check_interval=1)
...
class TestAutoupdateControl:
    ...
    async def test_check_and_perform_autoupdate_no_update(self, update_handler, handler_mocks):
        ...
        # should stay None as no update was triggered and current version was not None
        assert update_handler.last_check_version is None
    ...
    async def test_check_and_perform_autoupdate_initial(self, update_handler, handler_mocks):
        ...
        await update_handler._check_and_perform_autoupdate()
        assert update_handler.last_check_version == "12345"
    ...
    async def test_check_and_perform_autoupdate_with_update(self, update_handler, handler_mocks):
        ...
        # Manually patch cmd_update as an AsyncMock on the instance
        update_handler.cmd_update = AsyncMock()
        
        await update_handler._check_and_perform_autoupdate()
        
        # Should trigger update
        assert update_handler.cmd_update.called
...
class TestUpdateCommand:
    ...
    @pytest.mark.asyncio
    async def test_cmd_update_check_only(self, update_handler, handler_mocks):
        """Test update check without execution."""
        mock_message = Mock()
        mock_message.channel = Mock()
        ...
        update_handler._shutdown_and_update_servers = AsyncMock()
        
        with patch('asyncio.sleep', new_callable=AsyncMock):
            await update_handler.cmd_update(mock_message, ".update", ".update")
        
        assert update_handler._run_update_countdown.called
    
    @pytest.mark.asyncio
    async def test_cmd_update_with_execution(self, update_handler, handler_mocks):
        """Test update with execution."""
        mock_message = Mock()
        mock_message.channel = Mock()
    ...
    @pytest.mark.asyncio
    async def test_cmd_update_already_in_progress(self, update_handler, handler_mocks):
        """Test update when already in progress (in-memory lock)."""
        mock_message = Mock()
        mock_message.channel = Mock()
    ...
    @pytest.mark.asyncio
    async def test_cmd_update_stale_lock_file(self, update_handler, handler_mocks):
        """Test update removes a stale lock file."""
        mock_message = Mock()
        mock_message.channel = Mock()
    ...
    @pytest.mark.asyncio
    async def test_cmd_update_active_lock_file(self, update_handler, handler_mocks):
        """Test update respects an active lock file."""
        mock_message = Mock()
        mock_message.channel = Mock()
...
class TestForceUpdateCommand:
    """Test forceupdate command functionality."""
    
    @pytest.mark.asyncio
    async def test_cmd_forceupdate_all(self, update_handler, handler_mocks):
        ...
        with patch('patchraptor.update_management_handler.execute_steamcmd_simple', new_callable=AsyncMock) as mock_exec:
             ...
             with patch('asyncio.sleep', new_callable=AsyncMock):
                 with patch('patchraptor.update_management_handler.RaptorChatUtils.delayed_raptorchat_restart', new_callable=AsyncMock):
                     await update_handler.cmd_forceupdate(mock_message, ".forceupdate", ".forceupdate")
             
             assert mock_exec.called

    @pytest.mark.asyncio
    async def test_cmd_forceupdate_specific_map(self, update_handler, handler_mocks, mock_server):
        """Test forced update for a specific map."""
        mock_message = Mock()
        mock_message.channel = Mock()
        ...
        with patch('patchraptor.update_management_handler.execute_steamcmd_simple', new_callable=AsyncMock) as mock_exec:
             ...
             with patch('patchraptor.update_management_handler.RaptorChatUtils.delayed_raptorchat_restart', new_callable=AsyncMock):
                 await update_handler.cmd_forceupdate(mock_message, ".forceupdate TheIsland_WP", ".forceupdate theisland_wp")
             
             assert mock_exec.called
             handler_mocks['server_manager'].start_server.assert_called()

    @pytest.mark.asyncio
    async def test_cmd_forceupdate_server_not_found(self, update_handler, handler_mocks):
        """Test forced update when server is not found."""
        from patchraptor.exceptions import ServerNotFoundError
        mock_message = Mock()
        mock_message.channel = Mock()
        
        handler_mocks['server_manager'].find_server.side_effect = ServerNotFoundError("InvalidMap")
        
        await update_handler.cmd_forceupdate(mock_message, ".forceupdate InvalidMap", ".forceupdate invalidmap")
        
        handler_mocks['discord_manager'].send_temp_message.assert_called_with(
            mock_message.channel, "⚠️ No server found with name or map 'InvalidMap'."
        )
...
class TestInternalMethods:
    ...
    async def test_internal_shutdown_and_update_servers_success(self, update_handler, handler_mocks):
        ...
        update_handler._wait_for_servers_online = AsyncMock(return_value=True)
    ...
    async def test_internal_shutdown_and_update_servers_steamcmd_fail(self, update_handler, handler_mocks):
        ...
        with patch('patchraptor.update_management_handler.execute_steamcmd_simple', new_callable=AsyncMock) as mock_exec:
             ...
             handler_mocks['discord_manager'].send_temp_message.assert_called_with(
                 mock_channel, "❌ SteamCMD update failed. Manual intervention required."
             )
    ...
    async def test_internal_shutdown_and_update_servers_rcon_fail(self, update_handler, handler_mocks):
        ...
        assert any("Failed to send shutdown" in str(call) for call in handler_mocks['discord_manager'].send_temp_message.call_args_list)
        assert mock_exec.called
    ...
    async def test_run_update_countdown_webhook_no_msg(self, update_handler, handler_mocks):
        ...
        handler_mocks['discord_manager'].send_temp_message.assert_called_with(
            mock_channel, "⚠️ No webhook message configured for shutdown. Skipping Discord announcement."
        )
    ...
    async def test_internal_shutdown_and_update_servers_no_raptorchat_manager(self, update_handler, handler_mocks):
        ...
        mock_channel = Mock()
        update_handler.raptorchat_manager = None
        update_handler._restart_all_servers = AsyncMock()
        update_handler._wait_for_servers_online = AsyncMock(return_value=True)
    ...
    async def test_internal_shutdown_and_update_servers_online_fail(self, update_handler, handler_mocks):
        ...
        mock_channel = Mock()
        update_handler._restart_all_servers = AsyncMock()
        update_handler._wait_for_servers_online = AsyncMock(return_value=False)
    ...
    @pytest.mark.asyncio
    async def test_internal_shutdown_and_update_servers_webhook_success(self, update_handler, handler_mocks):
        """Test successful webhook call on completion."""
        mock_channel = Mock()
        update_handler._restart_all_servers = AsyncMock()
        update_handler._wait_for_servers_online = AsyncMock(return_value=True)
        ...
        handler_mocks['discord_manager'].send_webhook_message.assert_called_with("Servers are back!")
...
class TestWaitMethodsExtra:
    ...
    @pytest.mark.asyncio
    async def test_wait_for_servers_online_empty(self, update_handler):
        """Test waiting for an empty list of servers."""
        res = await update_handler._wait_for_servers_online([])
        assert res is True
    ...
    async def test_check_and_perform_autoupdate_no_channel(self, update_handler, handler_mocks):
        """Test autoupdate check when no notification channel is available."""
        update_handler.autoupdate_enabled = True
        handler_mocks['version_manager'].get_current_version.return_value = "12345"
        handler_mocks['version_manager'].get_latest_build_id.return_value = "67890"
        handler_mocks['discord_manager'].get_default_channel.return_value = None
        
        # Mock cmd_update to check if it's called
        update_handler.cmd_update = AsyncMock()
        
        await update_handler._check_and_perform_autoupdate()
        # Should return early without triggering update
        assert not update_handler.cmd_update.called
...
class TestUpdateErrors:
    ...
    @pytest.mark.asyncio
    async def test_cmd_update_exception_recovery(self, update_handler, handler_mocks):
        """Test cmd_update handles exceptions and cleans up lock file."""
        mock_message = Mock()
        mock_message.channel = Mock()
    ...
    @pytest.mark.asyncio
    async def test_forceupdate_rcon_failure(self, update_handler, handler_mocks, mock_server):
        """Test forceupdate when RCON shutdown fails."""
        mock_message = Mock()
        mock_message.channel = Mock()
        ...
        with patch('patchraptor.update_management_handler.execute_steamcmd_simple', new_callable=AsyncMock):
            await update_handler.cmd_forceupdate(mock_message, f".forceupdate {mock_server.name}", f".forceupdate {mock_server.name}")
        
        # Check if any call contains the error indicator
        assert any("Failed to shut down" in str(call) for call in handler_mocks['discord_manager'].send_temp_message.call_args_list)
...
class TestCmdAutoupdate:
    ...
    @pytest.mark.asyncio
    async def test_cmd_autoupdate_enable(self, update_handler, handler_mocks):
        """Test enabling autoupdate."""
        mock_message = Mock()
        mock_message.channel = Mock()
        
        update_handler.autoupdate_enabled = False
        
        await update_handler.cmd_autoupdate(mock_message, ".autoupdate on", ".autoupdate on")
        
        assert update_handler.autoupdate_enabled is True
    
    @pytest.mark.asyncio
    async def test_cmd_autoupdate_disable(self, update_handler, handler_mocks):
        """Test disabling autoupdate."""
        mock_message = Mock()
        mock_message.channel = Mock()
        
        update_handler.autoupdate_enabled = True
        
        await update_handler.cmd_autoupdate(mock_message, ".autoupdate off", ".autoupdate off")
