...
- **Performance Analytics**: View 7-day historical trends for player vitality, reliability, and hardware usage with `.analytics`.
...
## 3. 🔄 Server Updates
- **Maintenance Updates**: Schedule updates with automatic warning countdowns using `.patch`.
- **Emergency Updates**: Force immediate updates for critical patches via `.forcepatch`.
- **Version Checking**: Compare installed server versions against Steam's latest build with `.check`.
- **Auto-Patching**: Enable background checks that automatically apply new patches as soon as they are available (`.autopatch`).
...
- **Cloudflare Updater**: One-click utility for keeping the tunnel binary up to date (`update_cloudflared.bat`).
